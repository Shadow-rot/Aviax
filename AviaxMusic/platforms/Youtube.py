...
DL=Path("downloads");CK=Path("cookies");DL.mkdir(exist_ok=True)
AE=("mp3","m4a","webm");VE=("mp4","webm","mkv");MS=250;AR=10;CS=16384
def gck()->Optional[str]:
 if not CK.exists():return None
 c=list(CK.glob("*.txt"));return str(random.choice(c))if c else None
def cc(v:str,e:Tuple[str,...])->Optional[str]:
 for x in e:
  p=DL/f"{v}.{x}"
  if p.exists():return str(p)
 return None
async def apf(v:str,ab:str,dl:int)->Optional[str]:
 u=f"{ab}/{v}?api={API_KEY}"
 ...
 return None
async def dla(l:str)->Optional[str]:
 v=l.split('v=')[-1].split('&')[0];c=cc(v,AE)
 if c:return c
 return await apf(v,f"{API_URL}/song",4)
async def dlv(l:str)->Optional[str]:
 v=l.split('v=')[-1].split('&')[0];c=cc(v,VE)
 if c:return c
 return await apf(v,f"{VIDEO_API_URL}/video",8)
async def gsz(l:str)->Optional[int]:
 ck=gck()
...
class YouTubeAPI:
 ...
 async def url(s,m:Message)->Optional[str]:
  ...
  return None
 async def _ft(s,l:str,v:bool=False)->Dict:
 ...
 async def details(s,l:str,v:bool=False)->Tuple:d=await s._ft(l,v);return d["title"],d["dur"],d["dur_sec"],d["thumb"],d["id"]
 async def title(s,l:str,v:bool=False)->str:return(await s._ft(l,v))["title"]
 async def duration(s,l:str,v:bool=False)->str:return(await s._ft(l,v))["dur"]
 async def thumbnail(s,l:str,v:bool=False)->str:return(await s._ft(l,v))["thumb"]
 ...
 async def video(s,l:str,v:bool=False)->Tuple[int,str]:
  l=(s.B+l if v else l).split("&")[0];dl=await dlv(l)
  if dl:return 1,dl
  ck=gck()
 ...
 async def formats(s,l:str,v:bool=False)->Tuple[List[Dict],str]:
  l=(s.B+l if v else l).split("&")[0];ck=gck()
  if not ck:return[],l
  o={"quiet":True,"cookiefile":ck};yd=yt_dlp.YoutubeDL(o)
  try:
 ...
 async def download(s,l:str,my,video:bool=False,videoid:bool=False,songaudio:bool=False,songvideo:bool=False,format_id:Optional[str]=None,title:Optional[str]=None)->Union[Tuple[str,bool],str,Tuple[None,None]]:
  ...
  def yd(o:Dict)->str:
   ...
   y=yt_dlp.YoutubeDL(o);i=y.extract_info(l,download=False);ou=DL/f"{i['id']}.{i['ext']}"
   if ou.exists():return str(ou)
   y.download([l]);return str(ou)
  if songaudio or songvideo:return await dla(l)or f"downloads/{l}.mp3"
  if video:
   dl=await dlv(l)
